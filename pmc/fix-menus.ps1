# Fix all screen menus to use actual navigation instead of stubs

$screenFiles = Get-ChildItem "$PSScriptRoot/screens/*.ps1" | Where-Object {
    (Get-Content $_.FullName -Raw) -match 'Write-Host.*not implemented'
}
...
foreach ($file in $screenFiles) {
    ...
    # Find the menu setup block - match from "# Tasks menu" to end of Help menu
    $pattern = '(?s)(        # Tasks menu.*?Write-Host "PMC TUI v1\.0" \}\)\))'

    if ($content -match $pattern) {
        $newContent = $content -replace $pattern, $menuTemplate
        Set-Content -Path $file.FullName -Value $newContent -NoNewline
        Write-Host "  ✓ Updated $($file.Name)"
    } else {
        Write-Host "  ✗ Could not find menu block in $($file.Name)"
    }
}
